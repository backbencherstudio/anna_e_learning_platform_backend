...
export class FeedbackController {
  ...
  create(@Req() req: any, @Body() createFeedbackDto: CreateFeedbackDto, @UploadedFile() file?: Express.Multer.File) {
    const userId = req.user?.userId || req.user?.id;
    return this.feedbackService.create(userId, createFeedbackDto, file);
  }
  ...
  findAll(
    ...
    return this.feedbackService.findAll(userId, pageNum, limitNum, search, course_id, week_number, type, status);
  }
  ...
  update(@Req() req: any, @Param('id') id: string, @Body() updateFeedbackDto: UpdateFeedbackDto, @UploadedFile() file?: Express.Multer.File) {
    const userId = req.user?.userId || req.user?.id;
    return this.feedbackService.update(userId, id, updateFeedbackDto, file);
  }
