    ...
    try {
      switch (job.name) {
        ...
        case 'sendVerificationLink':
          ...
          break;
        case 'sendCardGeneratorEmail':
          this.logger.log('Sending card generator email');
          await this.mailerService.sendMail({
            to: job.data.to,
            from: job.data.from,
            subject: job.data.subject,
          ...
          break;
        case 'sendStudentNotificationEmail':
          this.logger.log('Sending student notification email');
          await this.mailerService.sendMail({
            to: job.data.to,
            from: job.data.from,
            subject: job.data.subject,
